...
namespace Snake_Final
{
    ...
    {
        int mapNum = 0;
        ConnMongo conn; //server mongo drop
        Map mapGame = new Map();
        ...
        public SoundPlayer audiofx;
        public string ace = "./Audio/2.wav";
        public string point = "./Audio/point.wav";
        ...
        /// <summary>
        /// food = 0 => +10 điểm, +1 thân [Đỏ]
        /// food = 1 => +5 điểm, đảo phím 3s [Xanh]
        /// food = 2 => +5 điểm, tăng tốc 6s [Sét]
        /// food = 3 => +5 điểm, giảm tốc 4s [Trắng]
        /// </summary>
        ...
        private void randomFood()
        {
            ...
            if (food.Length == 1)
            {
                int x = rnd.Next(Source.N);
                int y = rnd.Next(Source.M);
                int foodNum = rnd.Next(4);

                if (x == food[0].x && y == food[0].y) return;
            ...
            else
            {
                for (int i = 0; i < 2; i++)
                {
                    food[i].x = rnd.Next(Source.N);
                    food[i].y = rnd.Next(Source.M);
        ...
        private void Form1_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == kPause && pnl_pause.Visible == false && keyPause == true)
            {
            ...
            {
                if (dir == 3 && e.KeyCode == kUp) return;
                if (dir == 2 && e.KeyCode == kRight) return;
                if (dir == 1 && e.KeyCode == kLeft) return;
                if (dir == 0 && e.KeyCode == kDown) return;

                keyDown = true;
                int oldDir = dir;

                if (e.KeyCode == kUp) dir = 0;
                if (e.KeyCode == kDown) dir = 3;
                if (e.KeyCode == kLeft) dir = 2;
                if (e.KeyCode == kRight) dir = 1;
                if (oldDir == dir) return;
            }
            else
            {
                if (dir == 0 && e.KeyCode == kUp) return;
                if (dir == 1 && e.KeyCode == kRight) return;
                if (dir == 2 && e.KeyCode == kLeft) return;
                if (dir == 3 && e.KeyCode == kDown) return;

                keyDown = true;
                int oldDir = dir;
                if (e.KeyCode == kUp) dir = 3;
                if (e.KeyCode == kDown) dir = 0;
                if (e.KeyCode == kLeft) dir = 1;
                if (e.KeyCode == kRight) dir = 2;
                if (oldDir == dir) return;
            }
        }
        ...
        private void Dead(string msg)
        {
            timer_game.Stop();
            keyDown = true;
            pnl_gameover.Enabled = true;
            ScoreHint = 0;
            if (mapNum == 0) ScoreHint = score;
            else
            {
                for (int i = 0; i < mapNum; i++)
                {
                    ScoreHint += Map.Maps[i].Score;
                    if (i == mapNum - 1) ScoreHint += score;
                }
            }
            lbOK.Visible = false;
            label3.Text = "Joke over you're dead!";
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            {
                timer_game.Enabled = !timer_countdown.Enabled;
            }

            if (score >= Map.Maps[mapNum].Score)
            {
                //score = 0;
                ...
                for (int i = 0; i < num; i++)
                {
                    s[i].x = s[i].y = 0;//set vị tí qua map mới là 0,0
                }
                dir = 2;  // set hướng qua map mới là 2 (go Phải)              
            }
            //1s = 1000 ms
            for (int i = 0; i < buffList.Length; i++)
            {
                if (buffList[i].countTime == 0) continue;
                buffList[i].countTime += timer_game.Interval;
                ...
                if ((s[0].x == food[i].x) && (s[0].y == food[i].y))
                {
                    ...
                    else if (listFood[i] == 1)
                    {
                        ...
                        for (int z = 0; z < buffList.Length; z++)
                        {
                            ...
                            {
                                buffList[z].countTime = timer_game.Interval;
                                buffList[z].buff = 1;
                                rotateKey = true;
                                break;
                            }
        ...
        private void picEdit_Click(object sender, EventArgs e)
        {
            ...
            if (radioButton1.Checked)
            {
                ...
                b.TileSet = Binary.ConvertImage(mapGame.TilesetSprite[hScrollBar1.Value]);
                b.x = (e2.X) / 40;
                b.y = (e2.Y) / 40;
            ...
            if (radioButton2.Checked)
            {
                if (checkBox1.Checked == false)
                {
                    ...
                    tg.TileSet = Binary.ConvertImage(mapGame.MapTele[hScrollBar1.Value]);
                    tg.DrawX = (e2.X) / 40;
                    tg.DrawY = (e2.Y) / 40;
        ...
        private void picEditTile_Paint(object sender, PaintEventArgs e)
        {
            if (radioButton1.Checked)
            {
                Image Tile = Binary.ConvertImage(mapGame.TilesetSprite[hScrollBar1.Value]);
            ...
            if (radioButton2.Checked)
            {
                Image Tile = Binary.ConvertImage(mapGame.MapTele[hScrollBar1.Value]);
                Rectangle cropBg = new Rectangle(0, 0, Tile.Width, Tile.Height);
                e.Graphics.DrawImage(Tile, cropBg, 0, 0, Tile.Width, Tile.Height, GraphicsUnit.Pixel);
            }
        }
        ...
        private void btn_close_resume_Click(object sender, EventArgs e)
        {
            pnl_pause.Visible = false;
            pnl_pause.Enabled = false;
            timer_game.Start();
        ...
        private void timer_countdown_Tick(object sender, EventArgs e)
        {
            ...
            if (lb_countdown.Text == "START")
            {
                ...
                keyPause = true;
                keyDown = false;
                lb_countdown.Text = "countdown";
                countdown = 5;
                timer_game.Enabled = true;
            }
            countdown = countdown - 1;
            lb_countdown.Text = countdown < 0 ? "START" : countdown.ToString();
        ...
        {
            mapGame.spawn(e, mapNum);
            string mc = Map.Maps[mapNum].Score.ToString();
            lblScoreMap.Text = "Next area\n " + $"{score}"+"/"+mc;
            for (int i = num - 1; i >= 0; i--)
            {
                ...
                {
                    e.Graphics.DrawImage(Source.snakeBody[dir], cropHead1, 0, 0, 160, 160, GraphicsUnit.Pixel);
                }
            }
        ...
        private void lblOkBXH_Click(object sender, EventArgs e)
        {
            //conn.ImportBXH(textBox1.Text, ScoreHint);
            lbOK.Visible = false;
            pictureBox1.Focus();
            keyDown = true;
        ...
        private void Game_Load(object sender, EventArgs e)
        {
            ...
            Enum.TryParse(Gamesettings.Default.escape, out kPause);
            Enum.TryParse(Gamesettings.Default.up, out kUp);
            Enum.TryParse(Gamesettings.Default.down, out kDown);
            Enum.TryParse(Gamesettings.Default.left, out kLeft);
            Enum.TryParse(Gamesettings.Default.right, out kRight);
            randomFood();
            mapGame.CheckMaps();
            mapGame.LoadMap();
            //conn = new ConnMongo();
            timer_countdown.Start();
            lb_scoreboard.Text = "";//conn.LoadBXH();
            lblScoreMap.Text = Map.Maps[mapNum].Score.ToString();
            pnl_gameover.Visible = false;
            textBox1.MaxLength = 10;


            //conn.ImportBXH("You", 33);
            pnl_pause.Visible = false;
